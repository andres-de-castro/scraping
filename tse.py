...
with open('tse.csv', 'w') as dataFile:
    dataFile.write('Stock, Date, Open, High, Low, Close, Volume\n')
...
def transform_data(body, url=''):
    ...
    if len(timeSeries) > 0:
        df = pd.DataFrame.from_csv(StringIO(timeSeries),
                                    sep=",", parse_dates=False,
                                    header=None, index_col=None)

        df.drop(df.columns[[-1]], axis=1, inplace=True)
        df.columns = ['Date', 'Open', 'High', 'Low', 'Close', 'Volume']
        df.index = [code for _ in df.index]
        df.tail(1).to_csv('tse.csv', mode='a', header=None)
    else:
        sys.stderr.write(str(code) +
                         ' ' + ' has returned 0 values. check if deprecated\n')
